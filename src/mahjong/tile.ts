...
export const ORPHAN_TILES: Tile[] = ['1b','9b','1d','9d','1c','9c','Ew','Sw','Ww','Nw','Rd','Gd','Wd'];
...
export function is258(tile: Tile): boolean {
  const v = tile[0];
  return tile.length === 2 && (v === '2' || v === '5' || v === '8');
}

export function numValue(tile: Tile): number {
  return parseInt(tile[0]);
}

export function suit(tile: Tile): string {
  return isHonor(tile) ? (isDragon(tile) ? 'dragon' : 'wind') : tile[1];
}
